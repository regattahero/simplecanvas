...
- The pull request with this branch's changes was merged into
  ...
  from regattahero/feature/toolbar-overhaul"). Upstream `main` and this fork's
  `feature/toolbar-overhaul` are now identical in content.
- New work continues on this fork regardless — e.g. `Update-2` branches off
  `feature/toolbar-overhaul` for further changes not yet sent upstream.
